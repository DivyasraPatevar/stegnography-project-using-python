...
def detect_media_spam_alerts(file_name: str, issues: list) -> list:
    alerts = []
    if any(keyword in file_name.lower() for keyword in ["free", "download", "click", "urgent", "hack", "crack", "mod", "promo", "virus", "malware"]):
        alerts.append("File name uses spammy or bait-like keywords.")
    if any("deepfake" in issue.lower() or "fake" in issue.lower() or "synthetic" in issue.lower() for issue in issues):
        alerts.append("Media analysis detected deepfake or synthetic content indicators.")
...
def render_threat_summary(media_type: str, file_name: str, filename_issues: list, hidden_apk_flag: bool, risk: dict, spam_alerts: list, show_level: bool = True):
    st.markdown("---")
    ...
    if show_level:
        ...
        if risk['issues'] or spam_alerts or hidden_apk_flag:
            st.markdown("**Action:** Treat this as suspicious and validate before use.")
        else:
            st.markdown("**Action:** Lower risk, but still remain cautious.")


def generate_threat_report(media_type: str, label: str, score: float, issues: list, file_name: str, filename_issues: list, hidden_apk_flag: bool, spam_alerts: list) -> str:
    report_lines = [
    ...
    if page == "Landing Page":
        ...
        if file:
            ...
            if "image" in file_type:
                ...
                manipulation_flag = detect_ai_manipulation(image)
                spam_alerts = detect_media_spam_alerts(file_name, issues)
                if spam_alerts or filename_issues or hidden_apk_flag:
                ...
                render_threat_summary("image", file_name, filename_issues, hidden_apk_flag, media_risk, spam_alerts, show_level=False)
                if issues:
                    st.markdown("**Deepfake / image analysis notes:**")
                    for note in issues:
                        st.write(f"- {note}")
                report_target_image = image
                report_text = generate_threat_report("image", label, score, issues, file_name, filename_issues, hidden_apk_flag, spam_alerts)
                ...
                spam_alerts = detect_media_spam_alerts(file_name, issues)
                if spam_alerts or filename_issues:
                ...
                filename_risk = compute_media_risk_summary(
                    "video", fake, score, False, False, filename_issues
                )
                display_media_risk(filename_risk, show_level=False)
                render_threat_summary("video", file_name, filename_issues, False, filename_risk, spam_alerts, show_level=False)
                if issues:
                    st.markdown("**Video analysis notes:**")
                    for note in issues:
                        st.write(f"- {note}")
                report_text = generate_threat_report("video", label, score, issues, file_name, filename_issues, False, spam_alerts)
                ...
                audio_risk = compute_media_risk_summary(
                    "audio", fake, score, False, False, filename_issues
                )
                display_media_risk(audio_risk, show_level=False)
                render_threat_summary("audio", file_name, filename_issues, False, audio_risk, spam_alerts, show_level=False)
                if issues:
                    st.markdown("**Audio analysis notes:**")
                    for note in issues:
                        st.write(f"- {note}")
                report_text = generate_threat_report("audio", label, score, issues, file_name, filename_issues, False, spam_alerts)
